...
export default function TodoList() {
  ...
  const startEdit = (todo: Todo) => {
    ...
    if (todo.remindAt) {
      ...
      if (!todo.isRepeatable) setSpecificDate(new Date(todo.remindAt).toISOString().split('T')[0]);
    }
  ...
  const resetForm = () => {
    setEditingTodoId(null); setTitle(''); setDesc(''); setRemindTime('12:00');
    setSpecificDate(''); setSelectedDays([]);
  };
        ...
        <div class="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div class="bg-white rounded-[2.5rem] p-10 w-full max-w-md shadow-2xl space-y-6">
            ...
            <div class="flex gap-3">
              <button 
                onClick={() => {setActiveShareTodo(null); setShareEmail('')}} 
                class="flex-1 py-4 font-bold text-slate-400"
              >Cancel</button>
              <button 
                onClick={() => api.post('/todo/share', {todoId: activeShareTodo(), email: shareEmail()}).then(() => {alert("Submited!"); setActiveShareTodo(null)})} 
      ...
      <div class="max-w-7xl mx-auto p-6 lg:p-12 space-y-10">
        ...
        <header class="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
          <div>
            ...
            <p class="text-slate-400 font-medium">Welcome, <span class="text-slate-900 font-bold">{user()?.fullName}</span></p>
          </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-10">
              ...
              <form onSubmit={handleSave} class="space-y-6">
                ...
                <div class="space-y-4">
                  <div class="flex justify-between items-center">
                    <span class="text-sm font-black text-slate-400 uppercase tracking-widest">Repeatable</span>
                ...
                <div class="relative flex py-2 items-center">
                    ...
                    <span class="shrink mx-4 text-slate-300 font-bold text-xs uppercase">Or</span>
                    <div class="grow border-t border-slate-100"></div>
                </div>

                <div class="space-y-2">
                  <label class="text-[10px] font-black text-slate-400 uppercase ml-2">One-time date</label>
                  <input 
                    type="date" 
                    min={todayStr}
                    value={specificDate()}
                ...
                <div class="flex gap-2 pt-4">
                  <button type="submit" class="flex-1 bg-slate-900 text-white py-5 rounded-[1.5rem] font-black text-lg hover:bg-blue-600 transition-all active:scale-95 shadow-xl shadow-slate-200">
                    {editingTodoId() ? 'Edt' : 'Enter'}
                  </button>
                  <Show when={editingTodoId()}>
                    <button type="button" onClick={resetForm} class="px-8 bg-slate-100 text-slate-400 rounded-[1.5rem] font-bold hover:bg-slate-200">X</button>
                  </Show>
                </div>
              </form>
            </div>
          ...
          <div class="lg:col-span-7 space-y-4">
            <Show when={!loading()} fallback={<div class="py-20 text-center font-bold text-slate-300 animate-pulse">Loading...</div>}>
              <For each={todos()} fallback={<div class="py-20 text-center text-slate-300 font-black uppercase tracking-[0.2em]">You don't have todo</div>}>
                {(todo) => (
                  <div class={`group p-6 bg-white rounded-[2.5rem] border border-slate-100 transition-all flex flex-col gap-4 ${todo.isCompleted ? 'bg-slate-50/50 opacity-50' : 'hover:border-blue-200 shadow-sm hover:shadow-md'}`}>
                    <div class="flex justify-between items-start">
                      <div class="flex gap-5">
                        ...
                        >
                          <Show when={todo.isCompleted}><svg width="16" height="16" fill="none" stroke="currentColor" stroke-width="4" viewBox="0 0 24 24"><path d="M20 6 9 17l-5-5"/></svg></Show>
                        </button>
                        <div>
                          <h3 class={`text-xl font-bold leading-tight ${todo.isCompleted ? 'line-through text-slate-300' : 'text-slate-800'}`}>{todo.title}</h3>
                          <Show when={todo.description}><p class="text-slate-400 text-sm mt-1">{todo.description}</p></Show>
                        </div>
                      </div>
                      
                      <div class="flex gap-1 md:opacity-0 group-hover:opacity-100 transition-all">
                        <button onClick={() => setActiveShareTodo(todo.id)} class="p-2 text-slate-300 hover:text-blue-500 hover:bg-blue-50 rounded-xl">
                           <svg width="20" height="20" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path d="M4 12v8a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2v-8"/><path d="m16 6-4-4-4 4M12 2v13"/></svg>
                        </button>
                        <button onClick={() => startEdit(todo)} class="p-2 text-slate-300 hover:text-orange-500 hover:bg-orange-50 rounded-xl">
                        ...
                        <button onClick={() => deleteTodo(todo.id)} class="p-2 text-slate-300 hover:text-red-500 hover:bg-red-50 rounded-xl">
                    ...
                    <div class="flex flex-wrap gap-2 border-t pt-4 border-slate-50">
                      <Show when={todo.remindAt}>
                        <span class="px-3 py-1 bg-blue-50 text-blue-600 rounded-lg text-[10px] font-black uppercase tracking-tighter">
                          ...
                          {!todo.isRepeatable && ` (${new Date(todo.remindAt!).toLocaleDateString()})`}
                        </span>
                      </Show>
                      <Show when={todo.isRepeatable && todo.repeatDays}>
                        <span class="px-3 py-1 bg-orange-50 text-orange-600 rounded-lg text-[10px] font-black uppercase tracking-tighter">
                          🔁 {todo.repeatDays?.split(',').map(d => weekDays.find(wd => wd.id === +d)?.label).join(' ')}
                        </span>
                      </Show>
